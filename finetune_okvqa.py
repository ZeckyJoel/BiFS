...
def parse_args():
    ...
    parser.add_argument('--coco_path', type=str, default="/QA-Prompts/OK-VQA-COCO2014")
    ...
    parser.add_argument('--output_log_path', type=str, default="/QA-Prompts/logs/ok_output_log.txt")
    parser.add_argument('--eval_step', type=int, default=4, help="eval every 1/eval_step epoch")
    parser.add_argument('--dataset', type=str, default='okvqa', choices=['okvqa'])
...
def prepare_inputs_outputs(args, data):
    ...
    if args.use_KeyReason:        
        text_input = [
            f"Question: {question}\n\n{key_reason}\n\nOutput:" 
            for question, key_reason in zip(questions, key_reasons)
        ]
    else:
        text_input = [
            f"Question: {question}\n\nOutput:"
    ...
    if args.use_knowledge:
        ...
        knowledge_list = [s.rstrip('.') for s in data["knowledge"]]

        text_output = [
                f"Because of {knowledge}, so the answer is {answer}." 
                for knowledge, answer in zip(knowledge_list, open_answer_texts)
            ]
    else:
...
if __name__ == '__main__':
    ...
    from datasets.okvqa_dataset_2 import OKVQADataset
    if args.dataset == 'okvqa':
        train_dataset = OKVQADataset(data_path = "/QA-Prompts/MY_DATA/OK_0.38/ok_train.json", img_path = os.path.join(args.coco_path, 'train2014'), split="train")
        val_dataset = OKVQADataset(data_path = "/QA-Prompts/MY_DATA/OK_0.38/ok_val.json", img_path = os.path.join(args.coco_path, 'val2014'), split="val")
