...
from transformers import AutoTokenizer, AutoModelForCausalLM, AutoConfig, BitsAndBytesConfig, AutoProcessor, BlipForImageTextRetrieval
import torch
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", attn_implementation="flash_attention_2", customized_config=None, **kwargs):
    ...
    blip_model = BlipForImageTextRetrieval.from_pretrained(
        "/data/fangwenlong/A-Simple-Baseline-For-Knowledge-Based-VQA-main/blip-itm-base-coco")
    blip_processor = AutoProcessor.from_pretrained(
        "/data/fangwenlong/A-Simple-Baseline-For-Knowledge-Based-VQA-main/blip-itm-base-coco")
    blip_model = blip_model.to(device)
    #blip_processor = blip_processor.to(device)
    ...
    return tokenizer, model, image_processor, context_len,blip_model,blip_processor
